...
void servo_angulo(uint gpio, float angle)
{
...
void mover_suave(uint gpio, uint start_duty, uint end_duty, int step, uint delay_ms)
{
    uint slice = pwm_gpio_to_slice_num(gpio);
...
int main()
{
    ...
    while (1)
    {
        ...
        mover_suave(SERVO_PIN, duty_0_degrees, duty_180_degrees, 5, 10); // 0° → 180°
        sleep_ms(1000);                                                  // Aguarda 1 segundo
        mover_suave(SERVO_PIN, duty_180_degrees, duty_0_degrees, 5, 10); // 180° → 0°
        sleep_ms(1000);                                                  // Aguarda 1 segundo
    }
}